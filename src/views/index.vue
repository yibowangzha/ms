<template>
  <div class="cxy_box">
    ...
    <div v-for="(i,k) in arr" :key="k">
      <!-- 精品课程 -->
      <div v-if="i.channel_info.type == 1">
        <p class="cxy_page">
          <span></span>
          {{i.channel_info.name}}
        </p>
        <div class="cxy_jingpin">
          <ul class="cxy_item">
            ...
            >
            <!-- 看一下报名是否为1，1就报名0不报 -->
              <div class="cxy_pos" v-show="item.has_buy == 1">
       ...
       <!-- 名师阵容 -->
      <div v-if="i.channel_info.type == 3">
        <p class="cxy_page">
          <span></span>
          {{i.channel_info.name}}
        </p>
        <div class="cxy_content">
          <ul class="cxy_item">
            <!-- 跳到讲师详情 -->
            <li v-for="(item, index) in i.list" @click="sh(item.teacher_id)" :key="index">
              <div>
